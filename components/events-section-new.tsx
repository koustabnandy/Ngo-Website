...
const EventsSection = () => {
  ...
  const containerVariants = {
    ...
    visible: { 
      ...
      transition: { 
        ...
        delayChildren: isMobile || prefersReducedMotion ? 0.1 : 0.2
      }
    }
  };
  ...
  const itemVariants = {
    hidden: { 
      y: isMobile || prefersReducedMotion ? 0 : 20, 
      opacity: 0 
    },
    visible: { 
      y: 0, 
        ...
        >
          <Tabs defaultValue="past" className="w-full max-w-6xl mx-auto">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 mb-6 dark:bg-gray-800 h-12 rounded-full overflow-hidden border border-blue-100 dark:border-gray-700 p-1">
              ...
              >
                <motion.span 
                  whileHover={{ scale: isMobile || prefersReducedMotion ? 1.02 : 1.05 }} 
                  transition={{ 
                    type: isMobile || prefersReducedMotion ? "tween" : "spring", 
                    stiffness: 400, 
                    damping: 10,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 py-8">
                        {getCurrentEvents().map((event, index) => (
                          <motion.div 
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ 
                              delay: isMobile || prefersReducedMotion ? index * 0.05 : index * 0.1, 
                              duration: isMobile || prefersReducedMotion ? 0.3 : 0.5 
                            }}
                            whileHover={
